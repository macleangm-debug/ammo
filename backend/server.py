...
from routes import partners_router, flagging_router
...
class FlaggingRule(BaseModel):
    ...
    name: str
    description: str
    category: str  # quantity, frequency, profile, compliance, location, time, dealer, risk
    enabled: bool = True
    conditions: dict = {}
    severity: str = "medium"  # low, medium, high, critical
    auto_review: bool = False  # Automatically send to review queue
    created_at: str = Field(default_factory=lambda: datetime.now(timezone.utc).isoformat())
    updated_at: Optional[str] = None


class FlaggedTransaction(BaseModel):
    """Record of a flagged transaction"""
    flag_id: str = Field(default_factory=lambda: f"flag_{uuid.uuid4().hex[:12]}")
    transaction_id: str
    rules_triggered: List[str] = []
    severity: str = "medium"
    auto_review_created: bool = False
    review_id: Optional[str] = None
    resolved: bool = False
    resolved_by: Optional[str] = None
...
async def evaluate_flagging_rules(transaction: dict, citizen_profile: dict, dealer_profile: dict) -> dict:
    ...
    """
    # Get rules from database or use defaults
    ...
    for rule in rules:
        ...
        try:
            ...
            # GEOGRAPHIC ANOMALY CHECK
                ...
                max_distance = conditions.get("max_distance_km", 200)
                # Simplified - would need actual distance calculation
                txn_lat = transaction.get("gps_lat")
                txn_lng = transaction.get("gps_lng")
                if txn_lat and txn_lng and citizen_profile:
...
async def flag_transaction(transaction_id: str, flagging_result: dict) -> Optional[str]:
    ...
    Returns the flag_id if created.
    """
    ...
    # Create review item if auto_review required
    review_id = None
...
@api_router.get("/government/flagging-rules")
...
@api_router.put("/government/flagging-rules/{rule_id}")
...
@api_router.post("/government/flagging-rules")
...
@api_router.delete("/government/flagging-rules/{rule_id}")
async def delete_flagging_rule(rule_id: str, user: dict = Depends(require_auth(["admin"]))):
    """Delete a custom flagging rule (cannot delete default rules)"""
...
@api_router.get("/government/flagged-transactions")
...
@api_router.post("/government/flagged-transactions/{flag_id}/resolve")
async def resolve_flagged_transaction(flag_id: str, request: Request, user: dict = Depends(require_auth(["admin"]))):
    ...
    await db.flagged_transactions.update_one(
        ...
        {"$set": {
            ...
            "resolution_action": data.get("action", "cleared")  # cleared, escalated, blocked
        }}
    )
...
@api_router.post("/government/flagging/test-transaction")
async def test_flagging_rules(request: Request, user: dict = Depends(require_auth(["admin"]))):
    """Test flagging rules against a sample transaction without actually flagging"""
    data = await request.json()
    ...
    # Get profiles if real IDs provided
